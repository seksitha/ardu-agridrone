...
#define debug_can(level_debug, fmt, args...) do { AP::canMan().log_text(level_debug, "CANTester",  fmt, ##args); } while (0)
...
{
    if (driver_index >= AP::canMan().get_num_drivers() ||
        AP::canMan().get_driver_type(driver_index) != AP_CANManager::Driver_Type_CANTester) {
        return nullptr;
    }
