...
AP_RangeFinder_UAVCAN* AP_RangeFinder_UAVCAN::get_uavcan_backend(AP_UAVCAN* ap_uavcan, uint8_t node_id, uint8_t address, bool create_new)
{
    ...
    for (uint8_t i = 0; i < RANGEFINDER_MAX_INSTANCES; i++) {
        if ((RangeFinder::RangeFinder_Type)frontend.params[i].type.get() == RangeFinder::RangeFinder_Type::RangeFinder_TYPE_UAVCAN &&
        ...
        if (driver != nullptr && (driver->_backend_type == RangeFinder::RangeFinder_Type::RangeFinder_TYPE_UAVCAN)) {
...
void AP_RangeFinder_UAVCAN::update()
{
    ...
    } else if (_status == RangeFinder::RangeFinder_Status::RangeFinder_Good && new_data) {
        //copy over states
        state.distance_cm = _distance_cm ;
        state.last_reading_ms = _last_reading_ms;
        update_status();
        new_data = false;
    } else if (_status != RangeFinder::RangeFinder_Status::RangeFinder_Good) {
...
void AP_RangeFinder_UAVCAN::handle_measurement(AP_UAVCAN* ap_uavcan, uint8_t node_id, const MeasurementCb &cb)
{
    ...
    switch (cb.msg->reading_type) {
        case uavcan::equipment::range_sensor::Measurement::READING_TYPE_VALID_RANGE:
        {
            ...
            driver->_status = RangeFinder::RangeFinder_Status::RangeFinder_Good;
        ...
        {
            driver->_last_reading_ms = AP_HAL::millis();
            driver->_status = RangeFinder::RangeFinder_Status::RangeFinder_OutOfRangeLow;
            break;
        }
        case uavcan::equipment::range_sensor::Measurement::READING_TYPE_TOO_FAR:
        {
            driver->_last_reading_ms = AP_HAL::millis();
            driver->_status = RangeFinder::RangeFinder_Status::RangeFinder_OutOfRangeHigh;
            break;
        }
        default:
        {
